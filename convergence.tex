...
In tthis paper we assume that the dimensionality \(p\) satisfies
\(1<p<n-1\). Both \(p=1\) and \(p=n-1\) are excluded, because these
boundary cases of MDS have special properties which makes them
fundamentally different from MDS for the intermediate values of \(p\)
(De Leeuw (\citeproc{ref-deleeuw_E_23c}{2023}), De Leeuw et al.
...
\(+1\). For all \(X\) the matrix \(B(X)\) is also symmetric, positive
semi-definite, and doubly-centered.
...
\Gamma(X):=V^+B(X)X
\label{eq-guttmanx}
\end{equation} The \emph{SMACOF algorithm} for unconstrained MDS is,
...
iterations \eqref{eq-smacofx} tend (in a specific sense described later
...
\(V^+B(X)X=X\) says that the columns of \(X\) are eigenvectors of
\(V^+B(X)\), with eigenvalues equal to one. De Leeuw
(\citeproc{ref-deleeuw_U_14b}{2014}) shows that if \(V^+B(X)X=X\) and
...
De Leeuw, Jan. 2023. {``Least Squares Unidimensional Scaling.''}
