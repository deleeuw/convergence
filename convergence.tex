...
In (Euclidean, metric, least squares) multidimensional scaling (MDS) we
minimize the real valued loss function \begin{equation}
\sigma(X):=\frac12\mathop{\sum}_{1\leq i<j\leq n} w_{ij}(\delta_{ij}-d_{ij}(X))^2,
(\#eq:stress)
\end{equation} defined on \(\mathbb{R}^{n\times p}\), the space of all
\(n\times p\) matrices. We follow Kruskal
(\citeproc{ref-kruskal_64a}{1964}) and call \(\sigma(X)\) the
\emph{stress} of \emph{configuration} \(X\). Minimizing stress over
p-dimensional configurations is the MDS problem.

In @ref(eq:stress) the matrices of \emph{weights} \(W=\{w_{ij}\}\) and
\emph{dissimilarities} \(\Delta=\{\delta_{ij}\}\) are symmetric,
...
the \emph{configuration} \(X\), which are the coordinates of \(n\)
\emph{points} in \(\mathbb{R}^p\). Thus \(D(X)\) is also symmetric,
non-negative, and hollow.

We also follow De Leeuw (\citeproc{ref-deleeuw_C_77}{1977}) and assume,
without any real loss of generality, that \(W\) is \emph{irreducible},
so that the MDS problem does not separate into a number of smaller MDS
problems. For symmetric matrices irreducibility means that we cannot
find a permutation matrix \(\Pi\) such that \(\Pi'W\Pi\) is block
diagonal, i.e.~the direct sum of a number of smaller matrices.

\section{Notation}\label{notation}

\subsection{Configuration Space}\label{configuration-space}

First some convenient notation, first introduced in De Leeuw
(\citeproc{ref-deleeuw_C_77}{1977}). Vector \(e_i\) has \(n\) elements,
with element \(i\) equal to \(+1\), and all other elements zero.
\(A_{ij}\) is the matrix \((e_i-e_j)(e_i-e_j)'\), which means elements
\((i,i)\) and \((j,j)\) are equal to \(+1\), while \((i,j)\) and
\((j,i)\) are \(-1\). Thus \begin{equation}
...
Suppose \(Y_1,\cdots,Y_m\), with \(m\leq np\), are linear independent
elements of configuration space \(\mathbb{R}^{n\times p}\). They define
a basis for the \(m\)-dimensional subspace \(\mathbb{S}\).

We now require that the configuration \(X\) is a linear combination of
the matrices in the basis, i.e.~that \(X\in\mathbb{S}\). If \(m=np\)
then this requirement is not restriction and merely involves a linear
change of coordinates. But in general the constraint \begin{equation}
X=\sum_{s=1}^m\alpha_s Y_s
\end{equation} can be used to impose restrictions on the configuration
(Bentler and Weeks (\citeproc{ref-bentler_weeks_78}{1978}), De Leeuw and
Heiser (\citeproc{ref-deleeuw_heiser_C_80}{1980})).

We define \emph{coefficient space} \(\mathbb{R}^m\) as the space of all
vectors \(\alpha\). To define stress in coefficient space we need the
square, symmetric, positive semi-definite matrices \(\tilde A_{ij}\) of
order \(m\) with elements \begin{equation}
\tilde a^{ij}_{st}:=\text{tr}\ Y_s'A_{ij}Y_t.\label{eq-atildef}
\end{equation} Now \(d_{ij}^2(X)\) equals \begin{equation}
\tilde d_{ij}^2(\alpha):=\alpha'\tilde A_{ij}\alpha.\label{eq-dtildef}
\end{equation} Define \(\tilde V\) as in \eqref{eq-vdef} and
\(\tilde B(\alpha)\) as in \eqref{eq-bdef}, but with \(\tilde A_{ij}\)
instead of \(A_{ij}\). Thus \(\sigma(X)\) equals \begin{equation}
\tilde \sigma(\alpha):=1-\alpha'\tilde B(\alpha)\alpha+\frac12\alpha'\tilde V\alpha.\label{eq-stildef}
\end{equation} and the MDS problem in coefficient space is to minimize
\eqref{eq-stildef} over \(\alpha\in\mathbb{R}^m\).

\section{SMACOF}\label{smacof}

\subsection{Configuration Space}\label{configuration-space-1}
...
(\#eq:smacof)
\end{equation} De Leeuw (\citeproc{ref-deleeuw_C_77}{1977}) shows that
the SMACOF iterations @ref(eq:smacof) tend (in a specifc sense described
...
The algorithm was proposed by Guttman (\citeproc{ref-guttman_68}{1968}),
by differentiating stress and setting the derivatives equal to zero.
This ignores the problem that stress is not differentiable if one or
more distances are zero, and it also does not say anything about
convergence of the algorithm. In De Leeuw
(\citeproc{ref-deleeuw_C_77}{1977}) a new derivation of the algorithm
was given, using a majorization argument based on the Cauchy-Schwarz
inequality. This make it possible to avoid problems with
