...
The components of a (Euclidean, metric, least squares) multidimensional
scaling (MDS) problem are
...
\item
  ...
  \(\Delta\) of \emph{dissimilarities},
\item
  a symmetric, non-negative, and hollow matrix \(W\) of \emph{weights},
\item
  a list of \(m\) \emph{configurations} \(Y_1,\cdots,Y_m\) of dimension
  \(n\times r\).
\end{itemize}

we minimize the real valued loss
...
\(n\times p\) matrices. We call \(\sigma(X)\) the \emph{stress} of
\emph{configuration} \(X\). Minimizing stress over all \(p-\)dimensional
configurations (over \emph{configuration space}) is a classical metric
MDS problem (Kruskal (\citeproc{ref-kruskal_64a}{1964a}), Kruskal
(\citeproc{ref-kruskal_64b}{1964b})). The parameters of the problem are
the \(np\) elements of \(X\).

In \eqref{eq-stressx} the matrices of \emph{weights} \(W=\{w_{ij}\}\)
and \emph{dissimilarities} \(\Delta=\{\delta_{ij}\}\) are . The
matrix-valued function \(D(X)=\{d_{ij}(X)\}\) contains Euclidean
distances between the rows of the configuration \(X\), which are the
coordinates of \(n\) \emph{points} in \(\mathbb{R}^p\). Thus \(D(X)\) is
also symmetric, non-negative, and hollow.

We also assume, without any real loss of generality, that \(W\) is
\emph{irreducible}, so that the MDS problem does not separate into a
number of smaller MDS problems (De Leeuw
(\citeproc{ref-deleeuw_C_77}{1977})). For the symmetric matrix \(W\)
irreducibility means that we cannot find a permutation of rows and
columns that make the permuted \(W\) block-diagonal, i.e.~the direct
...
\(s,t=1,\cdots,m\) \(p,q=1,\cdots,r\) \(i,j,k,l=1,\cdots,n\)

For our purposes a slightly more general parametrization of stress is
more convenient. Suppose \(Y_1,\cdots,Y_m\), with \(m\leq np\), are
linear independent elements of configuration space
\(\mathbb{R}^{n\times p}\). We now parametrize the configuration \(X\)
as a linear combination \begin{equation}
X=\sum_{s=1}^m\alpha_s Y_s\label{eq-alpha}
\end{equation} of the matrices \(Y_s\) in the basis. We will study a
version of smacof parametrized using \emph{coefficient space}
\(\mathbb{R}^m\), the space of all vectors with the coefficients
\(\alpha\) of the linear combinations.

If \(m=np\) then the alternative parametrization \eqref{eq-alpha} is
merely a linear change of coordinates. Stress from \eqref{eq-stressx} is
the special case in which each of the \(np\) matrices \(Y_s\) have a
single element equal to one with all other elements equal to zero. If
\(m<np\) the constraint can be used to impose restrictions on the
configuration (Bentler and Weeks
(\citeproc{ref-bentler_weeks_78}{1978}), De Leeuw and Heiser
(\citeproc{ref-deleeuw_heiser_C_80}{1980})).

The parametrization in \eqref{eq-alpha} makes the distances, and thus
stress, a function of \(\alpha\). We will overload the symbols
\(\sigma\) and \(d\), and define stress as \begin{equation}
\sigma(\alpha):=
