...
The (Euclidean, metric, least squares, p-dimensional) multidimensional
scaling (MDS) problem is the minimization of the real valued loss
function \footnote{The symbol $:=$ is used for definitions.}
\begin{equation}
\sigma(X):=\mathop{\sum}_{1\leq i<j\leq n} w_{ij}(\delta_{ij}-d_{ij}(X))^2,
\label{eq-stressx}
\end{equation} over \emph{configurations} \(X\in\mathcal{X}\), where
\emph{configuration space} \(\mathcal{X}\) is a subset of
\(\mathbb{R}^{n\times p}\), the space of all \(n\times p\) matrices,
with \(1<p<n-1\). If \(\mathcal{X}\) is all of
\(\mathbb{R}^{n\times p}\) the MDS problem is \emph{unconstrained}, if
\(\mathcal{X}\) is a proper subset it is \emph{constrained}.

In \eqref{eq-stressx} the \emph{dissimilarities} \(\Delta\) and
\emph{weights} \(W\) are the \emph{data} of the problem. Both \(\Delta\)
and \(W\) are symmetric, non-negative, and
hollow\footnote{A hollow matrix has a zero diagonal.} matrices
\(\Delta\) of order \(n\). The \(d_{ij}(X)\) are Euclidean distances
between rows \(x_i\) and \(x_j\) of \(X\). Thus \begin{equation}
d_{ij}(X):=\sqrt{(x_{i}-x_{j})'(x_{i}-x_{j})}.\label{eq-ddef}
\end{equation} We call \(\sigma(X)\) the \emph{stress} of configuration
\(X\) (Kruskal (\citeproc{ref-kruskal_64a}{1964a}), Kruskal
(\citeproc{ref-kruskal_64b}{1964b})). Kruskal does not use weights in
his definition and calls our \eqref{eq-stressx} \emph{raw stress}. Since
he is only interested in non-metric MDS his definition of stress (or
\emph{normalized stress}) adds the sum of squared distances as a
denominator in \eqref{eq-stressx}.

In tthis paper we assume that the \emph{dimensionality} \(p\) satisfies
\(1<p<n-1\). Both \(p=1\) and \(p=n-1\) are excluded, because these
boundary cases of MDS have special properties which makes them
fundamentally different from MDS for intermediate values of \(p\) (De
Leeuw (\citeproc{ref-deleeuw_E_23c}{2023b}), De Leeuw et al.
(\citeproc{ref-deleeuw_groenen_mair_E_16e}{2016})). We also assume,
without loss of generality, that \(W\) is \emph{irreducible}, which
means in this context that the MDS problem does not separate into a
number of smaller MDS problems that can be solved independently (De
Leeuw (\citeproc{ref-deleeuw_C_77}{1977})).

\sectionbreak

\section{Notation}\label{notation}

First some convenient notation, introduced in De Leeuw
(\citeproc{ref-deleeuw_C_77}{1977}). A \emph{unit vector} \(e_i\) has
\(n\) elements, with element \(i\) equal to \(+1\), and all other
elements zero. \begin{equation}
  A_{ij}:=(e_i-e_j)(e_i-e_j)',\label{eq-adef}
\end{equation} which means elements \((i,i)\) and \((j,j)\) are equal to
\(+1\), while \((i,j)\) and \((j,i)\) are \(-1\). All other elements are
zero. With this definition \begin{equation}
d_{ij}^2(X)=\text{tr}\ X'A_{ij}X.
\label{eq-anot}
\end{equation}

We also define the matrix \begin{equation}
...
\end{equation} Matrix \(V\) is symmetric with non-positive off-diagonal
elements. It is doubly-centered (rows and columns add up to zero) and
thus weakly diagonally dominant. It follows that it is positive
semi-definite (see Varga (\citeproc{ref-varga_62}{1962}), section 1.5).
Because of irreducibility it has rank \(n-1\), and the vectors in its
null space are all proportional to \(e\), the vector with all elements
equal to \(+1\). For all \(X\) the matrix \(B(X)\) is also symmetric,
positive semi-definite, and doubly-centered.
...
Still following De Leeuw (\citeproc{ref-deleeuw_C_77}{1977}) we also
define \begin{align}
...
\end{equation} with equality if \(X=Y\).

Define the \emph{Guttman Transform} of configuration \(X\) as
\begin{equation}
\Gamma(X):=V^+B(X)X,
\label{eq-guttmanx}
\end{equation} with \(V^+\) the Moore-Penrose inverse of \(V\) given by
\begin{equation}
V^+=(V+\frac{1}{n}ee')^{-1}-\frac{1}{n}ee'.\label{eq-vinv}
\end{equation} Then, from \eqref{eq-SRE} and \eqref{eq-rhocs},
\begin{align}
  \sigma(X)&\leq 1-2\text{tr}\ X'B(Y)Y+\text{tr}\ X'VX=\notag\\
  &=1+\text{tr}\ (X-\Gamma(Y))'V(X-\Gamma(Y))
  -\text{tr}\ \Gamma(Y)'V\Gamma(Y),\label{eq-stressmaj}
\end{align} with equality if \(X=Y\). It follows that if we choose \(X\)
so that \begin{equation}
\text{tr}\ (X-\Gamma(Y))'V(X-\Gamma(Y))\leq\text{tr}\ (Y-\Gamma(Y))'V(Y-\Gamma(Y)),\label{eq-impmaj}
\end{equation} then \(\sigma(X)\leq\sigma(Y)\). If there is strict
inequality in \eqref{eq-impmaj} then \(\sigma(X)<\sigma(Y)\). In
particular if \(Y\neq\Gamma(Y)\) then
\(\sigma(\Gamma(Y))<1-\eta^2(\Gamma(Y))=\sigma(Y)\).

The \emph{SMACOF algorithm} for unconstrained MDS is, with a
superscripted iteration counter \(k\geq 1\), \begin{equation}
X^{(k)}=\Gamma(X^{(k-1)})=\Gamma^k(X^{0}),
\label{eq-smacofx}
\end{equation} where \(\Gamma^0=I\) and
\(\Gamma^k(X):=\Gamma(\Gamma^{k-1}(X))\). If during the iterations we
encounter an \(X\) for which \(X=\Gamma(X)\) we have finite convergence
and we stop. If \(p>1\) we can safely assume that the algorithm
generates an infinite sequence (i.e.~does not stop).

De Leeuw (\citeproc{ref-deleeuw_C_77}{1977}) shows that the SMACOF
iterations \eqref{eq-smacofx} converge (in a specific sense described
later in this paper) to a fixed point of the Guttman transform, i.e.~to
an \(X\) with \(\Gamma(X)=X\). If stress is differentiable at \(X\) then
the fixed points of the Guttman transform are stationary points, where
the derivative of stress vanishes. De Leeuw
(\citeproc{ref-deleeuw_A_84f}{1984}) shows that at a local minimum we
have \(d_{ij}(X)>0\) for all \((i,j)\) with \(w_{ij}\delta_{ij}>0\),
i.e.~stress is differentiable at local minima. The equation
\(\Gamma(X)=X\) says that the columns of \(X\) are eigenvectors of
\(V^+B(X)\), with eigenvalues equal to one. De Leeuw
(\citeproc{ref-deleeuw_U_14b}{2014}) shows that if \(\Gamma(X)X=X\) and
all eigenvalues of \(V^+B(X)\) are less than or equal to one then stress
has its global minimum at \(X\).

The SMACOF algorithm (without weights) was proposed by Guttman
(\citeproc{ref-guttman_68}{1968}) by differentiating stress and setting
the derivatives equal to zero. This derivation ignored the problem that
stress is not differentiable if one or more distances are zero, and it
also did not say anything about convergence of the algorithm. In De
...
algorithm was given, using the majorization argument based on the
Cauchy-Schwarz inequality. This made it possible to avoid problems with
differentiability, and it lead to a simple convergence proof.
...
The main result in De Leeuw (\citeproc{ref-deleeuw_C_77}{1977}), which
relies heavily on theory developed in Zangwill
(\citeproc{ref-zangwill_69a}{1969}), Ostrowski
(\citeproc{ref-ostrowski_73}{1973}), and Meyer
(\citeproc{ref-meyer_76}{1976}), is:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  The iterates \(X^{(k)}\) are in the compact set \(\eta(X)\leq 1\),
\item
...
\item
  \(\eta(X^{(k+1)}-X^{(k)})\) converges to zero and thus either
...
$\|x_{i+1}-x_i\|\rightarrow 0$ as $i\rightarrow\infty$.} and has one or
more accumulation points. Each accumulation point is a fixed point of
the Guttman transform. All accumulation points have the same function
value, which is also the limit of the stress sequence.

But it is important to emphasize that De Leeuw
(\citeproc{ref-deleeuw_C_77}{1977}) did not show that the sequence of
configurations \(\{X^{(k)}\}\) is actually a Cauchy sequence, and
consequently converges to a configuration \(X_\infty\). This is
basically because of the rotational indeterminacy of the MDS problem. If
\(K\) is a rotation matrix, i.e.~\(K'K=KK'=I\), then
\(d_{ij}(XK)=d_{ij}(X)\) for all \(i\) and \(j\), and thus
\(\sigma(XK)=\sigma(X)\). If \(X\) is a fixed point of the Guttman
transform, then \(\Gamma(XK)=\Gamma(X)K=XK\), and thus \(XK\) is a fixed
point as well. The rotations of \(X\) define a continuum. There are no
isolated fixed points, each fixed point is part of a nonlinear continuum
of fixed points. Local minima are never strict.

De Leeuw (\citeproc{ref-deleeuw_R_93c}{1993}) shows that stress has only
a single local maximum at \(X=0\), and consequently the only possible
stationary points are local minima and saddle points. It is shown by De
Leeuw (\citeproc{ref-deleeuw_E_19g}{2019}) that all fixed points of the
Guttman transform which are not of full column rank are saddle points.
Thus if \(X\) is a local minimizer of MDS in \(p\) dimensions, then
adding \(q\) zero columns to \(X\) produces a saddle point \([X\mid 0]\)
for MDS in \(p+q\) dimensions. At saddle points there are always
directions in which stress can be decreased, and consequently the SMACOF
algorithm with enough iterations will eventually get close to a
continuum of local minima (Lee et al.
(\citeproc{ref-lee_panageas_piliouras_simchowitz_jordan_recht_19}{2019})).
...
Ostrovski's Theorem (, theorem 22.1, p.~151 , Ortega and Rheinboldt
...
is. And, basically for the same reason, we have not shown that
X\^{}\{(k)\} converges to a fixed point at all. De Leeuw
...
his theorem 3 on page 175, but the proof is not very convincing,
although convincing enough to get past the reviewers. I will try to be
...
\(\partial\Gamma(X;Y)\) for the directional derivative at \(X\) in the
direction \(Y\). Partial derivatives can be obtained by choosing
\(Y=e_ie_s'\). Of course we assume here that \(d_{ij}(X)>0\) for all
\(i<j\) with \(w_{ij}\delta_{ij}>0\). \begin{equation}
...
\section{Rotated SMACOF}\label{rotated-smacof}

\subsection{Rotation to Principal
Components}\label{rotation-to-principal-components}
...
\end{equation} Let's call this rotated SMACOF, or rSMACOF for short.

It looks initialy as if rSMACOF requires a great deal of extra
computation, but this is actually not the case. If \(L\) is a rotation
matrix we have \(\Pi(XL)=\Pi(X)\) and \(\Gamma(XL)=\Gamma(X)L\). This
...
De Leeuw (\citeproc{ref-deleeuw_R_08b}{2008})
...
\subsection{Rotation to Lower Triangular
Form}\label{rotation-to-lower-triangular-form}

Instead of rotating to principal components we can also rotate to lower
triangular form, in which all \(\frac12 p(p-1)\) elements above the main
diagonal of \(X\) are zero.

De Leeuw (\citeproc{ref-deleeuw_E_23a}{2023a})

Suppose \(X_1\), the submatrix with the first \(p\) rows of \(X\), is
non-singular. We use the QR decomposition of the transpose of \(X_1\).
Thus \(X_1'=QR\) with \(Q\) square orthonormal and \(R\) upper
triangular. We have \(X_1=R'Q'\) and thus \(X_1Q=R'\). Therefor \(XQ\)
is lower triangular.
...
\] It is of some interest to look at the special case in which
\(Y_s=e_ke_p'\) and \(Y_t=e_le_q'\). Then \[
...
\section{Accelerated SMACOF}\label{accelerated-smacof}

Suppose \(Gamma(X)=X\) and \(\tau\neq 1\). Then
\(2\Gamma(\tau X)-\tau X=(2-\tau)X\) and
\(2\Gamma((2-\tau)X)-(2-\tau)X=\tau X\) Also
\(\sigma(\tau X)=\sigma((2-\tau)X)\)
...
De Leeuw, Jan. 1993. \emph{Fitting Distances by Least Squares}. Preprint
...
De Leeuw, Jan. 2008. \emph{{Derivatives of Fixed-Rank Approximations}}.
Preprint Series No. 547. UCLA Department of Statistics.
\url{https://jansweb.netlify.app/publication/deleeuw-r-08-b/deleeuw-r-08-b.pdf}.

\bibitem[\citeproctext]{ref-deleeuw_U_14b}
De Leeuw, Jan. 2014. {``{Bounding, and Sometimes Finding, the Global
...
De Leeuw, Jan. 2019. {``{Fitting Distances by Least Squares}.''}
\url{https://jansweb.netlify.app/publication/deleeuw-r-93-c/deleeuw-r-93-c.pdf}.

\bibitem[\citeproctext]{ref-deleeuw_E_23a}
De Leeuw, Jan. 2023a. {``Differentiating the QR Decomposition.''}
\url{https://jansweb.netlify.app/publication/deleeuw-e-23-a/deleeuw-e-23-a.pdf}.

\bibitem[\citeproctext]{ref-deleeuw_E_23c}
De Leeuw, Jan. 2023b. {``Least Squares Unidimensional Scaling.''}
\url{https://doi.org/10.13140/RG.2.2.27920.17927}.
...
De Leeuw, Jan, and Willem J. Heiser. 1977. {``Convergence of Correction
...
Gilbert, Paul, and Ravi Varadhan. 2019. \emph{{numDeriv: Accurate
...
\bibitem[\citeproctext]{ref-lee_panageas_piliouras_simchowitz_jordan_recht_19}
Lee, Jason D., Ioannis Panageas, Georgios Piliouras, Max Simchowitz,
Michael I. Jordan, and Benjamin Recht. 2019. {``First-Order Methods
Almost Always Avoid Strict Saddle Points.''} \emph{Mathematical
Programming} 176: 311--37.

\bibitem[\citeproctext]{ref-meyer_76}
Meyer, Robert R. 1976. {``{Sufficient Conditions for the Convergence of
...
Ortega, James M., and Werner C. Rheinboldt. 1970. \emph{{Iterative
Solution of Nonlinear Equations in Several Variables}}. Academic Press.

\bibitem[\citeproctext]{ref-ostrowski_73}
Ostrowski, Alexander M. 1973. \emph{Solution of Equations in Euclidean
and Banach Spaces}. Third Edition of Solution of Equations and Systems
of Equations. Academic Press.

\bibitem[\citeproctext]{ref-rudin_76}
Rudin, Walter. 1976. \emph{Principles of Mathematical Analysis}. Third
Edition. McGraw-Hill.
...
Varga, Richard S. 1962. \emph{{Matrix Iterative Analysis}}. Prentice
Hall.

\bibitem[\citeproctext]{ref-zangwill_69a}
Zangwill, Willard I. 1969. \emph{{Nonlinear Programming: a Unified
